...
if (typeof window.registerModule === 'function') {
  window.registerModule('patrulhamento', {
    onEnter: pat_init,
    onLeave: () => {
      console.log('[Patrulhamento] Saindo do módulo...');
      // Poderia limpar intervalos ou estados temporários aqui
    }
  });
}
...
function pat_obterGPS() {
  if (typeof window.gps_obterLocalizacao === 'function') {
    window.gps_obterLocalizacao();
  } else {
    alert('Modulo de GPS nao carregado.');
  }
}
